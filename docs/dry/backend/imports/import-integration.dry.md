...
1. **Endpoints de Importação**: Os endpoints RESTful são definidos no backend para receber arquivos de importação. Cada endpoint é associado a um perfil de importação específico, que define como os dados devem ser processados. Esta associação é feita em `src/lib/importer/profiles/index.ts`.
