...
interface ValidationError {
  type: 'missing_definition' | 'duplicate_definition' | 'broken_link' | 'format_error';
  message: string;
  file: string;
  line?: number;
  suggestion?: string;
...
class DryValidator {
  ...
  private definitions: Map<string, DryReference[]> = new Map();
  private references: Map<string, DryReference[]> = new Map();
  ...
  private async validateMarkdownLinks(filePath: string): Promise<void> {
    const content = fs.readFileSync(filePath, 'utf-8');
    const lines = content.split('\n');
    const relPath = path.relative(process.cwd(), filePath);
