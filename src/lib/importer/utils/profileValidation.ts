...
export function validateFields(profile: ImportProfile) {
  ...
  profile.fields.forEach((field, idx) => {
  ...
  if (profile.formato === "CSV") {
    const hasKey = profile.fields.some((f) => f.roles.includes("key"));
    ...
    requiredContexts.forEach((name) => {
      const found = profile.fields.some((f) => f.roles.includes("context") && f.name === name);
